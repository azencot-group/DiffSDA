...
def etth_data_loader(window_size=24, frame_ind=0, normalize="none"):
    all_files = glob.glob(os.path.join(DATASET_DIRS['etth'], '*.csv'))
...
def airq_data_loader(window_size=24, frame_ind=0, normalize="none"):
    """Function to load the Air Quality dataset into TF dataset objects
        ...
        Args:
            normalize: The type of data normalizatino to perform ["none", "mean_zero", "min_max"]
        """
    all_files = glob.glob(os.path.join(DATASET_DIRS['airq'], '*.csv'))
...
def physionet_data_loader(window_size=4, frame_ind=0, normalize='none', dataset='set-a'):
    ...
    data_dir = DATASET_DIRS['physionet']
    static_vars = ['RecordID', 'Age', 'Gender', 'Height', 'ICUType', 'Weight']

    processed_df_path = os.path.join(data_dir, 'processed_df.csv')
    processed_static_df_path = os.path.join(data_dir, 'processed_static_df.csv')
    if os.path.exists(processed_df_path):
        df_full = pd.read_csv(processed_df_path)
        df_static = pd.read_csv(processed_static_df_path)
    else:
        txt_all = list()
        ...
        df_full.to_csv(processed_df_path)
        df_static.to_csv(processed_static_df_path)
...
if __name__ == '__main__':
    trainset, validset, testset, _ = physionet_data_loader(window_size=4, frame_ind=0,
                                                           normalize="mean_zero")
